...
void print_usage()
{
    fprintf(stderr, "DEMO semigroup Sampled Gaussian kernel convolution  \n");
    fprintf(stderr, " gaussconv_sampled_kernel in sigma k(truncation) N(iterations) out \n");
}
...
int main(int argc, char **argv)
{
    ...
    // direct
    double s = sqrt((double)N)*sigma;
    gaussconv_sampled_kernel(x, y1, w, h, s, k);
    ...
    for(int i=0; i<N;i++)
    {
        gaussconv_sampled_kernel(t, y2, w, h, sigma, k);
        for(int j=0; j<w*h;j++)
            t[j] = y2[j];
    }
